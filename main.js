...
const connectToWhatsApp = async () => {
    ...
    nocache('./main', module => console.log(chalk.redBright('[ CMD ]  ') + time + chalk.yellowBright(` "${module}" Update!`)))
	...
	lexxy.ev.on('messages.upsert', async m => {
	    ...
	    require('./index')(lexxy, msg, m, _afk, setting, store, welcome, left, set_welcome_group, set_left_db, db_respon_list, set_proses, opengc, set_done, set_open, set_close)
	})
    ...
    lexxy.ev.on('connection.update', (update) => {
        ...
        if (connection === 'close') {
          ...
          if (update.receivedPendingNotifications) lexxy.sendMessage(`${ownerNumber}@s.whatsapp.net`, {text: '♨︎𝒷ℴ𝓉 𝓉ℯ𝓁𝒶𝒽 𝒷ℯ𝓇𝒽𝒶𝓈𝒾𝓁 𝓉ℯ𝓇𝒽𝓊𝒷𝓊𝓃𝓰 𝒷𝒶𝓃𝓰' })
    })
