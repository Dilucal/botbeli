...
const { webp2mp4File } = require('./lib/scrape/mediafire')
...
let simi = JSON.parse(fs.readFileSync('./database/simi.json'));
...
var packnamenya = 'Qynzuu'
var authornya = 'By Armand_'

module.exports = async(lexxy, msg, m, _afk, setting, store, welcome, left, set_welcome_group, set_left_db, db_respon_list, opengc, set_proses, set_done, set_open, set_close) => {
    try {
...
let { textdonasi } = require('./help')
        ...
        const sender = isGroup ? (msg.key.participant ? msg.key.participant : msg.participant) : msg.key.remoteJid
      	const isOwner = ownerNumber == sender ? true : ["6287823406145@s.whatsapp.net",`${ownerNumber}@s.whatsapp.net`].includes(sender) ? true : false
...
const isBan = cekBannedUser(sender, ban)
    ...
    const detikk = Math.floor( lebih % (1000 * 60) / 1000)
        ...
        const fdoc = {key : {participant : "0@s.whatsapp.net"},message: {documentMessage: {title:`Selamat ${ucapanWaktu} kak ${pushname}` ,jpegThumbnail: fs.readFileSync('./media/logo.jpg')}}}
        ...
        async function sendStickerFromUrl(from, url, packname1 = stc.packname, author1 = stc.author, options = {}) {
            ...
            exif.create(packname1, authorvar1, `sendstc_${names}`)
...
lexxy.groupParticipantsUpdate(from, [sender], "remove")
        ...
        // simi
if (isGroup && isSimi)
if (simi.includes(from)) {
fetchJson(`https://api.simsimi.net/v2/?text=${chats}&lc=id`)
...
if (!isCmd && checkResponGroup(from, chats, db_respon_group)) {
       ...
       /* const spampm = () => {
            ...
            msgFilter.addSpam(sender, lexxy.spam)
            if (msgFilter.isSpam(sender, lexxy.spam)) {
...
fs.writeFileSync('./database/user.json', JSON.stringify(pendaftar, null, 2))
        ...
        //  Anti spam
      //  msgFilter.ResetSpam(lexxy.spam)
        ...
        if (isGroup) {
            if (mentioned.length !== 0) {
                for (let i of mentioned) {
                    if (afk.checkAfkUser(i, _afk)) {
                        const getId = afk.getAfkId(i, _afk)
                        const getReason = afk.getAfkReason(getId, _afk)
                        const getTime = Date.now() - afk.getAfkTime(getId, _afk)
                        const heheh = ms(getTime)
                        await mentions(`@${i.split('@')[0]} sedang afk\n\n*Alasan :* ${getReason}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik lalu`, [i], true)
                        lexxy.sendMessage(i + "@s.whatsapp.net", {text: `Ada yang mencari anda saat anda offline\n\nNama : ${pushname}\nNomor : wa.me/${sender.split("@")[0]}\nIn Group : ${groupName}\nPesan : ${chats}`})
                    }
                    if (i === lexxy.user.id) {
...
const wiwik = `_*Selamat ${ucapanWaktu} kak ${pushname}*_
 ...
 *Jika Butuh Sesuatu Silahkan Ketik Perintah Dibawah Ini*
...
jika tidak muncul button ketik #help`
            var but = [
...
lexxy.sendMessage(from, ini_message_Menu_nya, { quoted: fkontak })
...
var listpriv = await store.chats.all().filter(v => v.id.endsWith('.net')).map(v => v)
var sections = [
{title: "━━━「 NuxL_MENU 」━━━", rows: [
{title: "𝐌𝐀𝐈𝐍 𝐌𝐄𝐍𝐔", rowId: prefix+"mainmenu", description: "Menampilkan Menu Main"},
{title: "𝐎𝐓𝐇𝐄𝐑𝐒 𝐌𝐄𝐍𝐔", rowId: prefix+"other", description: "Menampilkan Menu Others"},
{title: "𝐓𝐎𝐎𝐋𝐒 𝐌𝐄𝐍𝐔", rowId: prefix+"toolsmenu", description: "Menampilkan Menu Tools"},
{title: "𝐈𝐒𝐋𝐀𝐌𝐈𝐂 𝐌𝐄𝐍𝐔", rowId: prefix+"islm", description: "Menampilkan Menu Islamic"},
{title: "𝐒𝐓𝐈𝐂𝐊𝐄𝐑 𝐌𝐄𝐍𝐔", rowId: prefix+"stkr", description: "Menampilkan Menu Sticker"},
{title: "𝐑𝐀𝐍𝐃𝐎𝐌 𝐌𝐄𝐍𝐔", rowId: prefix+"rndm", description: "Menampilkan Menu Random"},
{title: "𝐏𝐑𝐄𝐌𝐈𝐔𝐌 𝐌𝐄𝐍𝐔", rowId: prefix+"premi", description: "Menampilkan Menu Premium"},
{title: "𝐒𝐓𝐎𝐑𝐄 𝐌𝐄𝐍𝐔", rowId: prefix+"storemenu", description: "Menampilkan Menu Store"},
{title: "𝐓𝐎𝐏𝐔𝐏 𝐌𝐄𝐍𝐔", rowId: prefix+"topupmenu", description: "Menampilkan Menu Topup"},
{title: "𝐆𝐑𝐎𝐔𝐏 𝐌𝐄𝐍𝐔", rowId: prefix+"grupmenu", description: "Menampilkan Menu Group"},
{title: "𝐆𝐀𝐌𝐄𝐒 𝐌𝐄𝐍𝐔", rowId: prefix+"gamemenu", description: "Menampilkan Menu Games"},
{title: "𝐎𝐖𝐍𝐄𝐑 𝐌𝐄𝐍𝐔", rowId: prefix+"ownermenu", description: "Menampilkan Menu Owner"},
{title: "𝐀𝐍𝐎𝐍𝐘𝐌𝐎𝐔𝐒 𝐂𝐇𝐀𝐓", rowId: prefix+"anon", description: "Menampilkan Menu Anonymous Chat"},
{title: "𝐒𝐄𝐀𝐑𝐂𝐇 𝐌𝐄𝐍𝐔", rowId: prefix+"serch", description: "Menampilkan Menu Search"},
{title: "𝐂𝐎𝐒𝐏𝐋𝐀𝐘 𝐌𝐄𝐍𝐔", rowId: prefix+"csply", description: "Menampilkan Menu Cosplayers"},
{title: "𝐀𝐍𝐈𝐌𝐄 𝐌𝐄𝐍𝐔", rowId: prefix+"anim", description: "Menampilkan Menu Anime"},
{title: "𝐂𝐄𝐊 𝐔𝐒𝐄𝐑𝐍𝐀𝐌𝐄 𝐆𝐀𝐌𝐄", rowId: prefix+"cekusr", description: "Menampilkan Menu Cek Username Game"},
{title: "𝐏𝐑𝐈𝐌𝐁𝐎𝐍 𝐌𝐄𝐍𝐔", rowId: prefix+"prmbn", description: "Menampilkan Menu Primbon"},
{title: "𝐌𝐀𝐊𝐄𝐑 𝐌𝐄𝐍𝐔", rowId: prefix+"mkr", description: "Menampilkan Menu Maker"},
{title: "𝐆𝐄𝐍𝐄𝐑𝐀𝐓𝐄 𝐌𝐄𝐍𝐔", rowId: prefix+"gnrt", description: "Menampilkan Menu Generate"},
{title: "𝐃𝐎𝐖𝐍𝐋𝐎𝐀𝐃 𝐌𝐄𝐍𝐔", rowId: prefix+"downloadmenu", description: "Menampilkan Menu Download"},
{title: "𝐄𝐃𝐈𝐓𝐎𝐑 𝐏𝐇𝐎𝐓𝐎 𝐌𝐄𝐍𝐔", rowId: prefix+"editmenu", description: "Menampilkan Menu Editor Photo"},
{title: "𝐄𝐍𝐂𝐎𝐃𝐄/𝐃𝐄𝐂𝐎𝐃𝐄 𝐌𝐄𝐍𝐔", rowId: prefix+"encd", description: "Menampilkan Menu Encode/Decode"},
{title: "𝐃𝐀𝐓𝐀𝐁𝐀𝐒𝐄 𝐌𝐄𝐍𝐔", rowId: prefix+"dtbs", description: "Menampilkan Menu Database"},
{title: "𝐀𝐃𝐃 𝐑𝐄𝐒𝐏𝐎𝐍/𝐌𝐄𝐒𝐒𝐀𝐆𝐄", rowId: prefix+"addresp", description: "Menampilkan Menu Respon"},
{title: "𝐒𝐄𝐓 𝐏𝐑𝐎𝐒𝐄𝐒/𝐃𝐎𝐍𝐄", rowId: prefix+"setprosesdone", description: "Menampilkan Menu proses/done"}
]}]
var listMessage = {
text: ` ╭━━━「 *DATA* 」
 ┊⫹ Bot : ${botName}
 ...
 ┊⫹ Balance : Rp.${getBalance(sender, _money)}
 ┊⫹ Limit Game : ${cekGLimit(sender, gcount, glimit)}
 ┊⫹ Jam : ${jam}
 ╰═┅═━––––––๑`,
footer: `© Powered By Armand X ${botName}`,
buttonText: "Tap!",
mentions: [sender], sections}
lexxy.sendMessage(from, listMessage, { quoted: fdoc })
/* const miaw = fs.readFileSync('./vn/miaw.mp3')
          lexxy.sendMessage(from, {audio: miaw, mimetype:'audio/mpeg', ptt:true }, )*/
break
case prefix+'mainmenu':
reply(require('./help').mainmenu(prefix))
break
case prefix+'other':
reply(require('./help').othersmenu(prefix))
break
case prefix+'toolsmenu':
reply(require('./help').convertmenu(prefix))
break
case prefix+'rndm':
reply(require('./help').randommenu(prefix))
break
case prefix+'premi':
reply(require('./help').premiummenu(prefix))
break
case prefix+'storemenu':
reply(require('./help').storemenu(prefix))
break
case prefix+'topupmenu':
reply(require('./help').topupmenu(prefix))
break
case prefix+'grupmenu':
reply(require('./help').grupmenu(prefix))
break
case prefix+'gamemenu':
reply(require('./help').gamemenu(prefix))
break
case prefix+'ownermenu':
reply(require('./help').ownermenu(prefix))
break
case prefix+'anon':
reply(require('./help').anonymousmenu(prefix))
break
case prefix+'serch':
reply(require('./help').searchingmenu(prefix))
break
case prefix+'downloadmenu':
reply(require('./help').downloadmenu(prefix))
break
case prefix+'editmenu':
reply(require('./help').imageeditor(prefix))
break
case prefix+'setprosesdone':
reply(require('./help').prosesdone(prefix))
break
case prefix+'addresp':
reply(require('./help').responmessage(prefix))
break
case prefix+'cekusr':
reply(require('./help').cekusername(prefix))
break
case prefix+'dtbs':
reply(require('./help').databasemenu(prefix))
break
case prefix+'gnrt':
reply(require('./help').generatemenu(prefix))
break
case prefix+'encd':
reply(require('./help').encodemenu(prefix))
break
case prefix+'prmbn':
reply(require('./help').primbonmenu(prefix))
break
case prefix+'anim':
reply(require('./help').animemenu(prefix))
break
case prefix+'csply':
reply(require('./help').cosplaymenu(prefix))
break
case prefix+'stkr':
reply(require('./help').stickermenu(prefix))
break
case prefix+'islm':
reply(require('./help').islamicmenu(prefix))
break
case prefix+'mkr':
reply(require('./help').makermenu(prefix))
break
case prefix+'help':{
const buttonsMenuu = [
{buttonId: `${prefix}donasi`, buttonText: { displayText: 'Donasi'}, type: 1},
{buttonId: `${prefix}owner`, buttonText: { displayText: 'Owner'}, type: 1},
{buttonId: `${prefix}db`, buttonText: { displayText: 'Dashboard'}, type: 1}
]
const buttonSmenuu = {
...
case prefix+"lapor":
  case prefix+"reportbug":
    case prefix+"bugreport":
      case prefix+"req":
  if (!q) return reply('[ ! ] Pesannya mana? Contoh Ketik › *#lapor Kak, Menu error.*') 
const kontil = (q)
                      if (kontil.length > 300) return reply('Maaf Teks Terlalu Panjang, Maksimal 300 Teks')

                    lexxy.sendMessage(`${ownerNumber}@s.whatsapp.net`, {text: `*[LAPORAN MASUK]*\nNomor : wa.me/${sender.split("@")[0]}\nPesan : ${kontil}\nBalas: Ketik #chat` })
...
break
case prefix+'ttp2': {
  if (!q) return reply(`Contoh : ${command} halo ${botName}`)
       let ane = await getBuffer(`https://api.lolhuman.xyz/api/ttp4?apikey=${lolkey}&text=${q}%F0%9F%91%8B`)
            ...
            await ffmpeg('./sticker/' + sender + '.png')
                ...
                .on('end', function () {
                    ...
                    exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
                        ...
                        lexxy.sendMessage(from, { sticker: fs.readFileSync(`./sticker/${sender}.webp`) }, { quoted: msg })
...
case prefix+'ttp': {
  if (!q) return reply(`Contoh : ${command} halo ${botName}`)
       let ane = await getBuffer(`https://miaw.my.id/api/maker/ttp?text=${q}&apikey=danzz`)
            fs.writeFileSync('./sticker/' + sender + '.png', ane)
            await ffmpeg('./sticker/' + sender + '.png')
                .input('./sticker/' + sender + '.png')
                .on('start', function (cmd) {
...
case prefix+'attp':{
            ...
            var data = await getBuffer(`https://miaw.my.id/api/maker/attp?text=${q}&apikey=danzz`)
            fs.writeFileSync('./sticker/' + sender + '.png', data)
            await ffmpeg('./sticker/' + sender + '.png')
                .input('./sticker/' + sender + '.png')
                .on('start', function (cmd) {
                    console.log(`Started : ${cmd}`)
...
case prefix+'stalkig':
                    if (!q) return reply(`Contoh: ${command} jessnolimit`)
                   var username = (q)
                   var data = await fetchJson(`https://miaw.my.id/api/stalker/instagram?username=${username}&apikey=miaw`)
                   var ini_result = data.result
                   var ini_buffer = await getBuffer(ini_result.profile)
                   var ini_txt = `Username : ${ini_result.username}\n`
                    ini_txt += `Full Name : ${ini_result.fullname}\n`
                    ini_txt += `Posts : ${ini_result.posts}\n`
                    ini_txt += `Followers : ${ini_result.followers}\n`
                    ini_txt += `Following : ${ini_result.following}\n`
                    ini_txt += `Bio : ${ini_result.bio}`
                    lexxy.sendMessage(from, {image : ini_buffer, caption: ini_txt })
                    break
...
case prefix+"tomp4": {
            if (isQuotedSticker) {
                await downloadAndSaveMediaMessage('sticker', `./media/${sender}.webp`)
                if (msg.message.extendedTextMessage.contextInfo.quotedMessage.stickerMessage.isAnimated === true) {
                    ...
                    exec(`convert ./media/${sender}.webp ${outGif}`, (err) => {
                        if (err) {
                            console.log(err)
                        ...
                        exec(`ffmpeg -i ${outGif} -vf "crop=trunc(iw/2)*2:trunc(ih/2)*2" -b:v 0 -crf 25 -f mp4 -vcodec libx264 -pix_fmt yuv420p ${outMp4}`, (err) => {
                            ...
                            lexxy.sendMessage(from, { video: { url: outMp4 }, caption: 'Done ~'}, {
                                quoted: msg,
                            }).then(() => {
                    ...
                    exec(`ffmpeg -i ./media/${sender}.webp ${ran}`, async (err) => {
                        fs.unlinkSync(`./media/${sender}.webp`)
                        if (err) return reply(mess.error)
                        await lexxy.sendMessage(from, { image: { url: ran }, caption: 'Done ~'}, {
                            quoted: msg,
                        })
...
case prefix+'toimg': case prefix+'toimage':
            ...
            if (isQuotedSticker) {
                await downloadAndSaveMediaMessage('sticker', `./tmp/sticker/${sender}.webp`)
                if (msg.message.extendedTextMessage.contextInfo.quotedMessage.stickerMessage.isAnimated === true) {
                    reply(mess.wait)
                    var outGif = getRandom('.gif')
                    var outMp4 = getRandom('.mp4')
                    exec(`convert ./tmp/sticker/${sender}.webp ${outGif}`, (err) => {
                        if (err) {
                            console.log(err)
                            fs.unlinkSync(`./tmp/sticker/${sender}.webp`)
                            return reply(`Error bruh`)
                        }
                        exec(`ffmpeg -i ${outGif} -vf "crop=trunc(iw/2)*2:trunc(ih/2)*2" -b:v 0 -crf 25 -f mp4 -vcodec libx264 -pix_fmt yuv420p ${outMp4}`, (err) => {
                            if (err) {
                    ...
                    reply(mess.wait)
                    var ran = getRandom('.png')
                    exec(`ffmpeg -i ./tmp/sticker/${sender}.webp ${ran}`, async (err) => {
                        fs.unlinkSync(`./tmp/sticker/${sender}.webp`)
                        if (err) return reply(mess.error)
                        await lexxy.sendMessage(from, { image: { url: ran }, caption: 'Done ~'}, {
                            quoted: msg,
...
.then((res) => lexxy.sendMessage(from, { text: 'Owner bot sangat *Ganteng*' }, {quoted: res}))
...
case prefix+'ytmp4': case prefix+'mp4':
		...
		var datah = await fetchJson(`https://miaw.my.id/api/downloader/ytmp4?url=${q}&apikey=danzz`)
...
case prefix+'ytmp3': case prefix+'mp3':
			   if (!q) return reply(`Kirim perintah ${command} https://youtube.com/watch?v=e00w_clLkS0`)
		var datah = await fetchJson(`https://api.lolhuman.xyz/api/ytaudio?apikey=Miawteam&url=${q}`)
		var data = datah.result
                var txtt = `*YOUTUBE DOWNLOADER*\n\n*≻ Title :* ${data.title}\n*≻ Channel :* ${data.channel}\n*≻ Viewers :* ${data.view}\n*≻ Size :* ${data.size}\n*≻ Url Source :* ${args[1]}\n\n_Sedang Mengirim Media..._`
                var teks = `Done!`
                lexxy.sendMessage(from, { image: { url: data.thumbnail }, caption: txtt }, { quoted: msg })
                lexxy.sendMessage(from, { audio: { url: data.link.link}, mimetype: 'audio/mpeg', fileName: `${data.title}.mp3` }, { quoted:msg })
                addCmd(command.slice(1), 1, commund)
			break
case prefix+'musik':{
...
var url = q
reply(mess.wait)
var soundcd = await fetchJson(`https://danzz-api.herokuapp.com/api/downloader/soundcloud?url=${q}&apikey=92b1a0h7ts`)
lexxy.sendMessage(from, { audio: { url: soundcd.result.download }, mimetype: 'audio/mpeg', fileName: `${q}`}, { quoted: msg})
}
break
case prefix+'play':
 if (!q) return reply(`contoh :\n${command} dj angel baby`)
...
if (isSimi) return reply(`Simi sudah aktif`)
                 	   simi.push(from)
                 	   fs.writeFileSync('./database/simi.json', JSON.stringify(simi))
                  	   reply(`Fitur Simi Aktif!`)
              	  } else if (args[1].toLowerCase() === "off") {
                  	  simi.splice(from, 1)
                 	   fs.writeFileSync('./database/simi.json', JSON.stringify(simi))
                 	    reply(`Fitur Simi Nonaktif`)
             	   } else {
...
👤 : Cukup Beli Di Armand Store Ketik *${prefix}creator*
...
case prefix+'sc':
case prefix+'script':{
reply(`Minta Ke Owner Kalo Di Kasih Beruntung`)
}
break
...
break
        ...
        case prefix+'bcgc': case prefix+'bcgroup': {
            if (!isPremium && !fromMe) return reply('Kamu bukan team miaw 🙏🏻')
...
var buttonMessageh = {
    ...
    text: `${q}`,
    footer: `© Powered By Qynzuu X ${botName}`,
    buttons: buttons,
    headerType: 1
    }
...
if (!q) return reply(`contoh :\n${command} username`)
var url = q
var fatihh = await fetchJson(`https://zenzapis.xyz/downloader/randomtiktok?apikey=7ef559c0595f&username=${q}`)
reply(mess.wait)
var fatih = fatihh.result.media
var usrnm = fatihh.result.username
var fllw = fatihh.result.followers
var txtny = `
username : ${usrnm}
followers : ${fllw}
caption : ${fatih.caption}
`
lexxy.sendMessage(from, { video: {url : fatih.videourl, mimetype: 'video/mp4'}, fileName: `${q}.mp4`, caption: `${txtny}` }, {quoted: msg})
// premium menu
case prefix+'anal':
  if (!isPremium) return reply(mess.OnlyPrem)
              let anl = await getBuffer(`https://api.lolhuman.xyz/api/random2/anal?apikey=${lolkey}`)
            fs.writeFileSync('./sticker/' + sender + '.png', anl)
            await ffmpeg('./sticker/' + sender + '.png')
                .input('./sticker/' + sender + '.png')
                .on('start', function (cmd) {
                    console.log(`Started : ${cmd}`)
...
case prefix+'yaoi':
  if (!isPremium) return reply(mess.OnlyPrem)
  var yaoi = await getBuffer(`https://api.lolhuman.xyz/api/random/nsfw/yaoi?apikey=${lolkey}`)
  reply(mess.wait)
  lexxy.sendMessage(from, { image : yaoi, caption : 'Done!'}, {quoted:msg })
  break
case prefix+'pentol': {
  if (!isPremium) return reply(mess.OnlyPrem)
...
case prefix+'hentai':
  if (!isPremium) return reply(mess.OnlyPrem)
  var ll = await fetchJson(`https://miaw.my.id/api/nsfw/hentai?apikey=danzz`)
  var data = ll.result.url
  reply(mess.wait)
  lexxy.sendMessage(from, { image : {url: data } , caption : `sange ya kak ${pushname}?`}, { quoted : msg })
  break
case prefix+'loli2':
  if (!isPremium) return reply(mess.OnlyPrem)
  var ll = await fetchJson(`https://miaw.my.id/api/nsfw/panties?apikey=danzz`)
  var data = ll.result.url
  reply(mess.wait)
  lexxy.sendMessage(from, { image : {url: data}, caption : `sange ya kak ${pushname}?`}, { quoted : msg })
  break
...
var url = q
var fatihh = await fetchJson(`https://miaw.my.id/api/downloader/tiktok?url=${q}&apikey=danzz`)
reply(mess.wait)
var fatih = fatihh.result
lexxy.sendMessage(from, { video: {url: fatih.video , mimetype: 'video/mp4', fileName: `${q}.mp4`}, caption: `${fatih.description}`}, {quoted: msg})
lexxy.sendMessage(from, { audio: {url: fatih.audio , mimetype: 'audio/mpeg' ,ptt:true}}, {quoted: msg})
break
case prefix+'tiktok':
if (!q) return reply(`contoh :\n${command} https://vt.tiktok.com/ZSdbFNn96/?k=1`)
var url = q
...
reply(mess.wait)
lexxy.sendMessage(from, { video: fatihh, mimetype: 'video/mp4', fileName: `${q}.mp4`, caption: `Video Nya Kak` }, {quoted: msg})
break
case prefix+"sosmed":{
...
case prefix+'jadwalsholat':{
  if (!q) return reply(`Contoh : ${command} banjarmasin`)
  var srh = await fetchJson(`https://api.lolhuman.xyz/api/sholat/${q}?apikey=${lolkey}`)
  var data = srh.result
  var txtsrh =
...
case prefix+'say':
case prefix+'tts':{
var tts = await getBuffer(`https://api.lolhuman.xyz/api/gtts/id?apikey=${lolkey}&text=${q}`)
if (!q) return reply(`Contoh :\n${command} hallo`)
reply(mess.wait)
lexxy.sendMessage(from, {audio: tts, mimetype:'audio/mpeg', ptt:true }, {quoted:msg})
}
